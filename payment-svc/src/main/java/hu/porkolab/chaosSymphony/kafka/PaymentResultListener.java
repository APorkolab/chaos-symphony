...
public class PaymentResultListener {
	...
	public void onResult(ConsumerRecord<String, String> rec) {
		try {
			...
			System.out.println("PaymentResult for orderId=" + orderId + " -> " + status);
			// TODO: if CHARGED → inventory.requested, else → compensate
		} catch (Exception e) {
			System.err.println("PaymentResult parse error: " + rec.value());
			e.printStackTrace();
		}
	}
}
